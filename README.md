...
The API is host-agnostic (plain Node, plus a `Dockerfile`). Options:

| Host | Cost | Notes |
|---|---|---|
| **Render** (recommended start) | Free tier | Auto-deploy from GitHub (set **root directory** to `server/`), env vars + health-check (`/api/health`) in the dashboard. Free instances sleep after ~15 min idle (30–60s cold start). |
| **Railway** | ~$5/mo usage | Great DX, no sleeping, auto-deploy from GitHub. |
| **Fly.io** | Pay-as-you-go | Docker-first (`server/Dockerfile`), regions worldwide, static IPs available. |

**Required server env vars** (see `server/.env.example`): `MONGODB_URI`, `JWT_SECRET` (use a long random string — `openssl rand -hex 32`), `CORS_ORIGINS` (e.g. `https://hb-source.github.io`), and the `SEED_*` values. After first deploy, run `npm run seed` once (host shell, or locally pointed at Atlas) to create the admin and demo data.
